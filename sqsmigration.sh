...
aws sqs list-queues --profile source_aws_account --region us-west-2 |grep -i http |sed -e 's/"//g'|sed -e 's/,//g' > urlfile ;aws sqs list-queues --profile ec --region us-west-2 |grep -i http |sed -e 's/"//g'|sed -e 's/,//g' |awk -F '/' '{print $NF}' > listname ; paste urlfile listname > urlname
